...
const initializeChannels = (data: { [key: string]: { [key: string]: string[] } }) => {
  const channels: {
    [key: string]: { open: boolean; subs: { [key: string]: { open: boolean; subs: string[] } } }
  } = {}
...
export default function PermDrawer({ onChannelSelect, selectedChannel, data }: PermDrawerProps) {
                ...
                <Collapse in={channels[main].open} timeout='auto' unmountOnExit>
                  <List component='div' disablePadding>
                    {Object.keys(channels[main].subs).map((sub) => (
                      <div key={sub}>
                        <ListItemButton sx={{ pl: 4 }} onClick={() => handleSubClick(main, sub)}>
                          <ListItemIcon>
                            <MailIcon />
                          </ListItemIcon>
                          <ListItemText primary={sub} />
                          {channels[main].subs[sub].open ? <ExpandLess /> : <ExpandMore />}
                        </ListItemButton>
                        <Collapse in={channels[main].subs[sub].open} timeout='auto' unmountOnExit>
                          <List component='div' disablePadding>
                            {channels[main].subs[sub].subs.map((subsub, index) => (
              ...
              {Object.keys(data[selectedChannel]).map((sub) => (
                ...
                >
                  ...
                  {data[selectedChannel][sub].map((subsub, index) => (
                    <Typography key={index} variant='body2' sx={{ pl: 2, mr: 10 }}>
                      {subsub}
                    </Typography>
                  ))}
